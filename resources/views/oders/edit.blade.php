                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('orders.update', $order->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="cake_id" name="cake_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach ($cakes as $cake)
                                    <option value="{{ $cake->id }}" {{ old('cake_id', $order->cake_id) == $cake->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="quantity" name="quantity" value="{{ old('quantity', $order->quantity) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" min="1">
                            @error('quantity')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" id="total_price" name="total_price" value="{{ old('total_price', $order->total_price) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" min="0">
                            @error('total_price')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
